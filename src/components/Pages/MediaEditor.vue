...
.row
  .xsmall-12.columns
    ...
    .view-container
      .media-cards
        Sortable.media-card(:list="form.collection", :element="'div'", :options="{animation: 50, group: {name: 'items', pull: 'clone', put: false}}")
          card(v-for="card, index in form.collection", :info="form.asKeyValue(card)", :key="card.id", :url="card.url", @select="select")
</template>
