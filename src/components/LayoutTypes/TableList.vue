      ...
      .columns
        .column.is-11
          ...
          span.is-primary(
            ...
            :class="{'is-active': $parent.sorters[field.name], 'asc': $parent.sorters[field.name] === 'ASC', 'desc': $parent.sorters[field.name] === 'DESC', 'is-sortable': JSON.parse(field.config).sortable}"
          ) {{ field.label }}
        .column.is-1
          span.filter-toggle.fa.is-small.pull-right(
            v-if="JSON.parse(field.config).searchable",
